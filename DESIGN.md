...
- **Warning Deep** (`{colors.warning-deep}` — `#F38800`): Pressed warning.
...
**`button-primary`** — the lime-green CTA control.
- Background `{colors.primary}`, text `{colors.on-primary}`, label `{typography.button-md}`, padding `{spacing.md} {spacing.xl}`, shape `{rounded.lg}` for large / medium and `{rounded.md}` for small.
...
- Background `{colors.canvas}`, text `{colors.ink}`, 1 px solid `{colors.ink}` border, same typography / padding, shape `{rounded.pill}`.
...
**`alert`** - the canonical inline callout.
- Background `{colors.canvas-soft}`, text `{colors.ink}` or `{colors.body}`, padding `{spacing.lg}`, gap `{spacing.lg}`.
- Compact alerts use full pill radius; expanded/action alerts use `{rounded.md}`.
- Tones: neutral `{colors.body}`, positive `{colors.positive-deep}`, negative `{colors.negative}`, warning `{colors.warning}`.

### Icons

**`icon`** - the canonical glyph layer.
- Figma source is the `Icons` component set.
- Code source is `src/components/ui/Icon.tsx`, backed by Expo `@expo/vector-icons/MaterialIcons`.
- Components must use the wrapper, not direct imports from icon libraries.
- Default icon size is `{spacing.xl}` and default color is `{colors.ink}`.

### Identity

**`avatar`** — the canonical person or group identity marker.
