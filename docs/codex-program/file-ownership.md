...
Waves 4, 5, 6, 7, 8, 9, 10, 11, and the Wave 12A backend clients preflight are merged into `codex/integration`. Wave 12 runtime backend domain rollout remains blocked until backend provider/base URL/credentials, server ownership rules, production auth provider, account/workspace model, credential policy, conflict policy, and privacy/retention decisions exist. Any further backend domain work must start in separate thread/worktree scopes that do not add provider-specific SDKs, runtime network rollout, app route changes, persistence migrations, auth provider SDKs, or production hardening until those decisions are accepted.
