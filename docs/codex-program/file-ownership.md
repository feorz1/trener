...
Waves 4, 5, 6, 7, 8, 9, 10, 11, Wave 12A backend clients preflight, and Wave 13 production hardening baseline are merged into `codex/integration`. Wave 12 runtime backend domain rollout remains blocked until backend provider/base URL/credentials, server ownership rules, production auth provider, account/workspace model, credential policy, conflict policy, and privacy/retention decisions exist. Final audit remains blocked until production signing/release, monitoring/crash reporting, privacy/retention/delete/export, and accessibility acceptance criteria are accepted. Any further backend or production runtime work must start in separate thread/worktree scopes and must not add provider-specific SDKs, runtime network rollout, app route changes, persistence migrations, auth provider SDKs, signing secrets, monitoring SDKs, or production config until those decisions are accepted.
